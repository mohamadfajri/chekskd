...
create index if not exists skd_scores_nama_prefix_idx
  on public.skd_scores (nama_normalized text_pattern_ops)
...
begin
  ...
  if v_nama is not null then
    v_sql := v_base_sql
      ...
      || $order$
        order by
          case when s.nama_normalized = $1 then 0 else 1 end,
          s.nama_normalized,
          s.no_peserta
        limit $5
      $order$;
...
comment on function api_private.search_public_skd_scores(text, text, text, text, integer) is
  'Internal public-data search with prefix-first ranking and contains fallback.';
