      ...
      GET: async ({ request }: { request: Request }) => {
        ...
        const [sessionRows, phoneCount, marketingCount, ...statusCounts] = await Promise.all([
          sb
            ...
            .select(
              "id, token, status, sender_wa_id, used_count, created_at, updated_at, delivered_at, failure_message, leads(nama_panggilan, target_instansi, target_formasi, recommendation_mode, consent_marketing)",
            )
